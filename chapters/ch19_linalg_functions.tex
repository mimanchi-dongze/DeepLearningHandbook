...
\begin{variable}
\begin{itemize}
    \item $A \in \mathbb{R}^{m \times n}$: 待分解的原始矩阵。
    \item $U \in \mathbb{R}^{m \times m}$: 左奇异向量矩阵，其列向量是 $AA^T$ 的特征向量。
    \item $\Sigma \in \mathbb{R}^{m \times n}$: 奇异值矩阵，对角线上的元素 $\sigma_i$ 为奇异值。
    \item $V \in \mathbb{R}^{n \times n}$: 右奇异向量矩阵，其列向量是 $A^T A$ 的特征向量。
\end{itemize}
\end{variable}
